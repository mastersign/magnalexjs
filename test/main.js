...
describe('MagnaLex', () => {
	...
	describe('Library.parseReference()', () => {
		...
		it('should parse german reference without translation', () => {
			...
			assert(r.chapterNo, 'No chapter number in referenec')
			assert(r.verseNo, 'No verse number in reference')
			assert.equal(r.bookName.name, '1. Mose')
			assert.equal(r.chapterNo, 2)
			assert.equal(r.verseNo, 3)
		})
		it('should parse english reference without translation', () => {
			...
			assert(r.chapterNo, 'No chapter number in referenec')
			assert(r.verseNo, 'No verse number in reference')
			assert.equal(r.bookName.name, 'Genesis')
			assert.equal(r.chapterNo, 2)
			assert.equal(r.verseNo, 3)
		})
	})
	...
	describe('Library.loadVerses()', () => {
		it('should load one verse from KJV', () => {
			...
			assert.equal(v.reference.bookName.langTag, 'en', 'Verse reference has wrong language')
			assert.equal(v.reference.bookName.name, 'John', 'Verse reference has wrong book name')
		...
		it('should load one verse from explicit default translation', () => {
			...
			assert.equal(_.size(verses), 1, 'Did not load one verse')
			const v = verses[0]
			assert.equal(v.reference.translation.shortName, 'LUT1912', 'Verse reference has wrong translation')
			assert.equal(v.reference.bookName.langTag, 'de', 'Verse reference has wrong language')
			assert.equal(v.reference.bookName.name, 'Johannes', 'Verse reference has wrong book name')
			assert.equal(v.reference.chapterNo, 3, 'Verse reference has wrong chapter number')
			assert.equal(v.reference.verseNo, 16, 'Verse reference has wrong verse number')
			assert(_.isString(v.text), 'Verse contains no text')
		})
